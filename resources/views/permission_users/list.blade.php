<table class="table-auto w-full"> 
    <thead>
        <tr class="bg-indigo-600 text-white">
            ...
            <th class="px-4 py-2">
                <button wire:click="create" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4">New Permission user</button>
            </th>
        </tr>
    </thead>
    <tbody>
        @foreach($permissionUsers as $registry)
            <tr>
                <td class="border px-4 py-2">{{$registry->Permission}}</td>
                <td class="border px-4 py-2">{{$registry->User}}</td>
